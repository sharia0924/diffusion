...
- 样本数 n=24, S_hide=50, S_rec=50, strength=1.0
- 载密图基线: PSNR 9.31 dB / SSIM 0.0991 / LPIPS n/a
- nonce 协议: `nonce_i = H(key || nonce_start+i)`（自包含, 不依赖 cover）

## A. 错密钥/近密钥 BER 分布

- 错密钥数 96（随机 40 + 近密钥变体）
- BER: mean **0.4903** / std 0.0289 / min 0.4297 / max 0.5755（期望 0.5，越集中越安全）
...
- 错密钥距离: mean 15.89 / std 0.59（期望 ≈ 16.0）
- FAR@tau=4: **0.00000**
- FAR@tau=8: **0.00000**
- FAR@tau=12: **0.00000**
...
- 无 nonce 协议: mean **0.186** / max 0.479
...
- 无 nonce 协议: **1.000**（direction +1, train 12 / test 12）
- 逐图 nonce（ours）: **0.917**（direction +1, train 12 / test 12）
