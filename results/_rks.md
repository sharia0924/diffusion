...
- 载密图基线: PSNR 10.03 dB / SSIM 0.1436 / LPIPS n/a
- nonce 协议: `nonce_i = H(key || nonce_start+i)`（自包含, 不依赖 cover）

## A. 错密钥/近密钥 BER 分布

- 错密钥数 36（随机 8 + 近密钥变体）
- BER: mean **0.4900** / std 0.0712 / min 0.2812 / max 0.6562（期望 0.5，越集中越安全）
...
- 错密钥距离: mean 15.57 / std 1.60（期望 ≈ 16.0）
- FAR@tau=4: **0.00000**
- FAR@tau=8: **0.00000**
- FAR@tau=12: **0.00000**
...
- 无 nonce 协议: mean **0.202** / max 0.285
- 逐图 nonce（ours）: mean **0.020** / max 0.037
