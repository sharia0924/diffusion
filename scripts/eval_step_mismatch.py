...
def main():
    ...
    print(f"stego PSNR {psnr(sg, covers):.2f} dB; decoder trained @ S_rec={trained_sr}")

    rows = []
    for sr in rec_list:
        a_c = decode_acc(stego, sg, keys, nonces, sr, dec, bits, args.batch)
        a_j = decode_acc(stego, sg_j, keys, nonces, sr, dec, bits, args.batch)
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        f.write(f"# 复原步数失配鲁棒性（解码器训练于 S_rec={trained_sr}, "
                ...
                f"- 载密图 PSNR(vs cover) = {psnr(sg, covers):.2f} dB\n"
